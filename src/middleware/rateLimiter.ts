 ...
 */
export const quoteLimiter = isDevelopment
  ? noOpLimiter
  : rateLimit({
      ...
      handler: (req: Request, res: Response) => {
        ...
        logger.warn(
          { ip, endpoint: "quote" },
          "Blocked quote request - rate limit exceeded",
        );

        res.status(429).json({
          error: "Too many requests",
          message:
            "You have exceeded the quote rate limit. Please try again later.",
          retryAfter: 60,
        });
      },
...
export const generalLimiter = isDevelopment
  ...
  : rateLimit({
      ...
      handler: (req: Request, res: Response) => {
        ...
        logger.warn(
          { ip, endpoint: "general" },
          "Blocked general request - rate limit exceeded",
        );
