...
export class PonderClient {
  ...
  private currentGraphqlUrl: string | null = null;
  ...
  /**
   * Common error handling logic for both REST and GraphQL requests
   */
  private handleRequestError(
    ...
    // Check if we should retry or if max retries exceeded
    if (shouldRetry && attempt < maxRetries) {
      return { shouldRetry: true, shouldThrow: false };
    }
  ...
  private async request<T = any>(
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      ...
      try {
        ...
        const { shouldRetry, shouldThrow } = this.handleRequestError(
          error,
          url!,
          method,
          path,
  ...
  /**
   * Execute a GraphQL query with the same retry and fallback logic as REST API calls
   */
  async query<T = any>(
    query: string,
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      ...
      try {
        ...
        if (!this.graphqlClient || this.currentGraphqlUrl !== graphqlUrl) {
          this.graphqlClient = new GraphQLClient(graphqlUrl, {
            headers: { "User-Agent": "API-Service/1.0" },
          });
          this.currentGraphqlUrl = graphqlUrl;
        }
        ...
        if (shouldRetry) {
          this.logger.info(`[Ponder] GraphQL retrying after ${retryDelay}ms`);
          await delay(retryDelay);
          continue;
        }
