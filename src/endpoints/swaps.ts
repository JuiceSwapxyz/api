 ...
 */
export function createSwapsHandler(
  routerService: RouterService,
  logger: Logger,
) {
  return async function handleSwaps(
    req: Request,
    res: Response,
  ): Promise<void> {
    ...
    if (!txHashes || !chainId) {
      log.debug(
        { txHashes, chainId },
        "Validation failed: missing txHashes or chainId",
      );
    ...
    if (txHashesArray.length === 0) {
      log.debug(
        { txHashes },
        "Validation failed: invalid txHashes (empty array)",
      );
    ...
    try {
      ...
      if (!provider) {
        res
          .status(400)
          .json({
            message: `No RPC provider available for chainId ${chainId}`,
          });
        return;
      }
      ...
      const statusCounts = swapResults.reduce(
        (acc, swap) => {
