...
export function createSwapHandler(
  ...
  return async function handleSwap(req: Request, res: Response): Promise<void> {
    const requestId =
      (req.headers["x-request-id"] as string) || `swap-${Date.now()}`;
    ...
    try {
      ...
      if (
        ...
        if (routingType) {
          return await handleGatewaySwap(
            body,
            res,
            log,
            routerService,
      ...
      return await handleClassicSwap(
        body,
        res,
        log,
        routerService,
  ...
  try {
    ...
    if (
      ...
      res.status(400).json({
        ...
        detail:
          "One token must be native currency (0x0000000000000000000000000000000000000000)",
      });
      return;
    }
    ...
    if (isWrap) {
      ...
      if (!isValidWrap) {
        res.status(400).json({
          ...
          detail:
            "For WRAP, tokenIn must be native currency and tokenOut must be wrapped token",
        });
        return;
      }
    } else if (isUnwrap) {
      const isValidUnwrap =
        ...
        validatedTokenOut.toLowerCase() ===
          NATIVE_CURRENCY_ADDRESS.toLowerCase();
      if (!isValidUnwrap) {
        res.status(400).json({
          error: "Invalid UNWRAP request",
          detail:
            "For UNWRAP, tokenIn must be wrapped token and tokenOut must be native currency",
        });
        return;
      }
    }
    ...
    if (isWrap) {
      ...
      transactionData = wethInterface.encodeFunctionData("withdraw", [
        amountBN,
      ]);
      value = "0x0";
    }
      ...
      gasEstimates: [
        {
          ...
          gasFee: gasFee.toString(),
          maxFeePerGas: gasPrices.maxFeePerGas.replace("0x", ""),
          maxPriorityFeePerGas: gasPrices.maxPriorityFeePerGas.replace(
            "0x",
            "",
          ),
        },
    ...
    log.debug(
      { type, amount, wrappedTokenAddress },
      "Wrap/unwrap transaction prepared",
    );
    res.json(response);
  } catch (error) {
...
async function handleGatewaySwap(
  ...
  try {
    ...
    if (routingType === Routing.GATEWAY_JUICE_IN) {
      ...
      if (tokenOut.toLowerCase() !== contracts.JUSD.toLowerCase()) {
        ...
        log.debug(
          ...
          "Gateway JUICE_IN swap prepared (step 1)",
        );
        res.json(swapData);
        return;
      }
      ...
      log.debug(
        { routingType, tokenIn, tokenOut },
        "Gateway JUICE_IN swap prepared",
      );
      res.json(swapData);
      return;
    }
    ...
    if (gatewayQuote.isDirectConversion) {
      ...
      const minAmountOut = userOutputBN
    ...
    const isNativeInput =
      tokenIn.toLowerCase() === NATIVE_CURRENCY_ADDRESS.toLowerCase();
    ...
    const swapData = {
      ...
      to: gatewayAddress,
      value: isNativeInput
        ? ethers.BigNumber.from(body.amount).toHexString()
        : "0x0",
      from: body.from,
      maxFeePerGas: gasPrices.maxFeePerGas,
      maxPriorityFeePerGas: gasPrices.maxPriorityFeePerGas,
...
async function handleClassicSwap(
  ...
  try {
    ...
    log.info(
      `Swap protocols - body.protocols: ${JSON.stringify(body.protocols)}, parsed: ${JSON.stringify(protocols)}`,
    );
    ...
    // Estimate gas for the swap transaction
    const gasPrice = await provider.getGasPrice();
    let gasLimit = ethers.BigNumber.from("300000"); // 300K - based on Citrea Mainnet P95 (230K) + 30% buffer
    try {
      const estimatedGas = await provider.estimateGas({
        to: routerAddress,
        from: body.from,
